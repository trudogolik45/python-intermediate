...
Разрешения собраны в `Permission` — строковый Enum, поэтому опечатка в праве ловится на входе, а не молча превращается в отказ доступа. Каждому эндпоинту задан минимальный набор прав декоратором `require_permissions`: просмотру товара нужен `view_product`, созданию — `add_product`, обновлению — `update_product`, удалению — `delete_product`; списку пользователей — `view_user`, списку файлов — `view_file`, загрузке — `upload_file`. Декоратор берёт пользователя из параметра `current_user`, который приходит зависимостью `get_current_user`, и сверяет требуемые права с его `permissions`; не хватает хотя бы одного — 403. Отдельной ветки для администратора нет: `AdminUser` создаётся с полным набором прав и проходит ту же проверку, что и все.
